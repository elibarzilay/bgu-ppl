...
(define (primitive-tests)
  ...
  (test (derive-eval '(cons 3 (cons 4 (list)))) => '(evaluator-list (3 4)))
  ;; New tests
  (test (derive-eval '(list 3 4)) => '(evaluator-list (3 4)))
...
(define (app-lambda-tests)
  ...
  (test (derive-eval '((lambda (x) (+ x 1)) 2)) => 3)
  (test (derive-eval '((lambda (x) ((lambda (x) (+ x 1)) 2)) 12)) => 3)
  (test (derive-eval '((lambda (f) (f 2 1)) +)) => 3)
  (test (derive-eval '((lambda (f x y) (f x y)) + 12 4)) => 16)
  (test (derive-eval '((lambda (f x y) (f x y)) ((lambda () +)) 12 4)) => 16)
  (test (derive-eval '((lambda (f) (f 4)) (lambda (x) (+ x x)))) => 8)
  (test (derive-eval '((lambda (g) (g)) (lambda () (* 3 2)))) => 6)
  (test (derive-eval '((lambda (x) x) ((lambda (x) x) 6))) => 6)
  (test (derive-eval '((lambda (f g) (f (g))) (lambda (x) x) (lambda () (* 3 2)))) => 6)
  ;; New tests
  (test (derive-eval '((lambda (lst) (car lst)) (list 11 12 13))) => 11))
...
(define (let-tests)
  ...
  (test (derive-eval '(let ((f (lambda (x) (+ x x))) (g (lambda (x) (* x 2)))) (f (g 3)))) => 12)
